...
# Convertir toutes les variables quantitatives en logarithmes
reuters_2023 <- reuters_2023 %>%
...
# calcul du rmse pour la base d'apprentissage (Approche naive)

RMSE_appr <-
  RMSE_appr_mediane %>%  
  mutate(
    erreur_mediane = (total - total_prev)^2  # 
  ) %>%  
  summarise(RMSE_mediane = sqrt(mean(erreur_mediane, na.rm = TRUE))) #21.5
...
RMSE_test <-
  ...
  summarise(RMSE_mediane = sqrt(mean(erreur_mediane, na.rm = TRUE))) #20.3
...
# Application de l'ols par la validation croisée avec caret
cv_model_ols <- train(
...
## TAF pour Lundi
# TAF comparer les coefficients lm et caret 
# Prendre la regression linéaire et comparer avec la mediane (voir la où il ya les erreurs de prédiction)
# Sur les coeffcients et sur les predictions
# Approche naive par le modèle par la médiane: origine des differences de prédiction
...
print(paste("RMSE: ", rmse)) #1.916

# Calcul du MAE ( Pourquoi il est  plus elevé que le RMSE)
mae_ols <- mean(abs(reuters_test_ols$total - prediction_ols)) #1.366587


# regresson linéaire avec la méthode standard te comparaison avec caret

ols_model <- train(x_appr_el_encoded, y_appr_el_encoded,  method = "lm") # préciser les variables explicatives et la variable dépendante
#estimation classique par la methode OLS
print(ols_model)            #la regression OLS a reduit le nombre de regresseurs en fonction du nombre 

ols_pred <- predict(ols_model, X_test_scaled)
ols_mse <- mean((ols_pred - y_test_el_encoded)^2)
ols_rmse <- sqrt(ols_mse) 
coef((ols_model$finalModel)) # les coefficients du modele ols 



# ML sous la regression quantile (elle estime l'effet de la variable explicative sur l'ensemble
# de la distribution de la variable d'intérêt (total = émission scope 12 2023)
install.packages("quantreg")
library(quantreg)

# Mise en hypothèse de la distribution normal dans les variables explicatives pour éviter les bruits

set.seed(123) 

reuters_appr_normal <- reuters_appr%>% 
  mutate(revenue_Musd = revenue_Musd + rnorm(n(), mean = 0, sd = 1e-6))

reuters_test_normal <- reuters_test %>% 
  mutate(revenue_Musd = revenue_Musd +rnorm(n(), mean = 0, sd = 1e-6 ))

# le modèle de régression quantile pour le 50ème percentile (médiane) 
reg_quantile <- rq(total ~ revenue_Musd + Region + gics2bis, data = reuters_appr_normal, tau = 0.5) # AO ici le tau modélise la médiane de la distribution
summary(reg_quantile)

# Faire des prédictions sur l'ensemble de test 

predictions_quantile <- predict(reg_quantile, newdata = reuters_test_normal)

# Calculer le RMSE

rmse_quantile <- sqrt(mean((reuters_test$total - predictions_quantile)^2)) 
print(paste("RMSE: ", rmse_quantile)) # Pas besoin de faire une regression quantile : on a deja une regression en ols(avec une imputation par la médiane)
                                      # Revoir le document de l'insee pour la pratique de la regression quantile
# Calculer le MAE 
mae_quantile <- mean(abs(reuters_test$total - predictions_quantile)) 
print(paste("MAE: ", mae_quantile)) # 1.338


# Conversion des variables catégorielles en facteur dans la base d'apprentissage et la base de test
reuters_appr <- reuters_appr %>%
  mutate(Region = as.factor(Region), gics2bis = as.factor(gics2bis))

reuters_test <- reuters_test %>%
  mutate(Region = as.factor(Region), gics2bis = as.factor(gics2bis))

# Modèles Ridge, Lasso et Elastic Net avec l'algorithme de caret
...
# Prédictions et calcul du RMSE sur la base de test avec l'algorithme de caret

# Prédictions et RMSE pour le modèle Ridge
ridge_pred <- predict(ridge_model, newdata = reuters_test)
ridge_rmse <- sqrt(mean((reuters_test$intensite_bis - ridge_pred)^2))
mae_ridge <- mean(abs(reuters_test$intensite_bis - ridge_pred))

# Prédictions et RMSE pour le modèle Lasso
lasso_pred <- predict(lasso_model, newdata = reuters_test)
lasso_rmse <- sqrt(mean((reuters_test$intensite_bis - lasso_pred)^2))
mae_lasso <- mean(abs(reuters_test$intensite_bis - lasso_pred))

# Prédictions et RMSE pour le modèle Elastic Net
elastic_net_pred <- predict(elastic_net_model, newdata = reuters_test)
elastic_net_rmse <- sqrt(mean((reuters_test$intensite_bis - elastic_net_pred)^2))
mae_en <- mean(abs(reuters_test$intensite_bis - elastic_net_pred))

# résultats
print(paste("RMSE Ridge - Test:", ridge_rmse)) #7.675
print(paste("MAE Ridge - Test:", mae_ridge))   # 7.322
print(paste("RMSE Lasso - Test:", lasso_rmse)) # 7.68
print(paste("MAE Lasso - Test:", mae_lasso))   # 7.320
print(paste("RMSE Elastic Net - Test:", elastic_net_rmse)) # 7.685
print(paste("MAE Elastic Net - Test:", mae_en)) # 7.321



# Comparaison des coefficients des algo caret vs lm 
